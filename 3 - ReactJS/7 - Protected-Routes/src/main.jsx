...
createRoot(document.getElementById("root")).render(
  // Encontra o elemento #root no HTML e renderiza a árvore React.
  <AuthProvider>
    {" "}
    {/* Torna o contexto de auth disponível para tudo dentro (Router, rotas, páginas). */}
    <Router>
      {" "}
      {/* Habilita roteamento via History API (SPA). */}
      <Routes>
        {" "}
        {/* Escolhe qual <Route> renderizar com base na URL atual. */}
        <Route
          path="/" // Caminho raíz ("/").
          element={
            // Elemento a ser renderizado quando a rota for acessada.
            // Envolve o conteúdo com o guardião de rota privada.
            <PrivateRoute>
              <Home />{" "}
              {/* Conteúdo protegido: só aparece se houver user no contexto. */}
            </PrivateRoute>
          }
        />
        <Route

...
export default function App() {
  ...
  return (
    // Retorna a árvore de elementos JSX com as rotas.
    <BrowserRouter>
      {" "}
      {/* Provedor de roteamento baseado na URL do navegador. */}
      <Routes>
        {" "}
        {/* "Seletor" de rotas que escolhe a melhor correspondência para a URL atual. */}
        <Route path="/" element={<RootLayout />}>
          {" "}
          {/* Rota PAI para "/" que renderiza o layout; filhos aparecem onde o layout usa <Outlet />. */}
          <Route index element={<Home />} />{" "}
          {/* Rota de ÍNDICE do pai: corresponde exatamente a "/". */}
          <Route path="sobre" element={<Sobre />} />{" "}
          {/* Rota FILHA relativa: forma a URL "/sobre". */}
          <Route path="posts/:id" element={<Post />} />{" "}
          {/* Rota dinâmica: "/posts/123" captura "id" via useParams() dentro de <Post />. */}
          <Route path="*" element={<NotFound />} />{" "}
          {/* Curinga: qualquer caminho não casado pelos filhos acima cai aqui (404). */}
        </Route>
      </Routes>
    </BrowserRouter>
  );
